                ...
                <nav className="relative px-4 py-4 flex justify-between items-center w-full">
                    <a className="text-3xl font-bold leading-none" href="/">
                        <svg version="1.0" xmlns="http://www.w3.org/2000/svg"
                            width="70" height="70" viewBox="0 0 500.000000 500.000000"
...

                    {
                        user ?
                            <DropdownMenu modal={false}>
                                <DropdownMenuTrigger className='select-none hidden lg:flex lg:ml-auto lg:mr-3 lg:items-center lg:gap-4'>
                                    <img src={user.picture || undefined} alt={user.name || undefined} className="rounded-full h-10 select-none" />
                                </DropdownMenuTrigger>
                                <DropdownMenuContent side='bottom' className=''>
                                    <DropdownMenuLabel className='select-none'>{user.nickname}</DropdownMenuLabel>
                                    <DropdownMenuSeparator />
                                    <a href="/profile">
                                        <DropdownMenuItem>
                                            Profile
                                    ...
                                    <DropdownMenuSeparator />
                                    <Button variant={'destructive'} className=' h-8 text-xs w-full'>
                ...
                <div className={`navbar-menu relative z-50 ${isMenuOpen ? 'lg:hidden' : 'hidden'}`}>
                    ...
                    <nav className="fixed top-0 left-0 bottom-0 flex flex-col w-5/6 max-w-sm py-6 px-6 bg-white border-r overflow-y-auto">
                        ...
                        <div>
                            <ul>
                                {mainMenu.map((menu, index) => (
                                    <li key={index} className='select-none mb-1'>
                                        <a className={menu.to === location ? 'font-bold p-4 text-blue-600' :
                                            'block p-4 text-sm font-semibold text-gray-400 hover:bg-blue-50 hover:text-blue-600 rounded'} href={`${menu.to}`}>{menu.title}</a>
                                    </li>
                                ))}
                            </ul>
                        </div>
